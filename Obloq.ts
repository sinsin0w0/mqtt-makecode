...
namespace Obloq {
    ...
    */
    ...
    //% SERVER.fieldEditor="gridpicker" SERVER.fieldOptions.columns=2
    //% blockId=setup
    //% block="Setup | Pin set: | receiving data (green wire): %receive| sending data (blue wire): %send | Wi-Fi: | name: %SSID| password: %PASSWORD| start connection: | Servers: %SERVER"
    export function setup(/*serial*/receive: SerialPin, send: SerialPin,
        /*wifi*/SSID: string, PASSWORD: string,
        /*connect*/SERVER: SERVERS):
        void {
        OBLOQ_WIFI_SSID = SSID
    ...
    //% SERVER.fieldEditor="gridpicker" SERVER.fieldOptions.columns=2
    //% blockId=login
    //% block="Login |IoT service: | Iot_id: %IOT_ID| Iot_pwd: %IOT_PWD| (default topic_0) Topic: %IOT_TOPIC "
    export function login(
        ...
        OBLOQ_MQTT_ID = IOT_ID
        OBLOQ_MQTT_TOPIC[0][0] = IOT_TOPIC
        Obloq_start_connect_mqtt(SERVERS.Stemhub, "connect mqtt")
    }
